...
class MatchingService
{
    ...
    public function generateShortlist(string $rfsId): array
    {
        ...
        // Include the top-level category itself - service types can be attached
        // directly to a top-level category (not nested in a subcategory), and
        // those must still be eligible for category-based partial matching.
        // Without this, a service type like "IT Consultation" (attached
        // directly to "Technology & IT") would never be considered for an RFS
        // requesting "Web App Development" (nested under "Software
        // Development", a child of "Technology & IT"), even though both
        // clearly belong to the same broad technology area.
        $subcategoryIds[] = $topLevelId;
